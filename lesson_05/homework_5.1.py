...
for car_name, car_info in car_data.items():
    ...
    price = car_info[4]
    if year >= search_criteria[0] and engine_volume >= search_criteria[1] and price <= search_criteria[-1]:
       #print(f'{car_name}: year {year}, engine volume {engine_volume}, price {price}')
       #filtered_cars = []
        filtered_cars.append( (car_name, year, engine_volume, price) )
        #print(filtered_cars)
sorted_cars = sorted(filtered_cars, key=lambda car_info: car_info[3])
...
#print(f'car_name: year {year >= 2017}, engine_volume {engine_volume >= 1.6}, price {price <= 36000}')
#car_data_tuple = tuple(car_data)
#print(car_data_tuple)
#new
